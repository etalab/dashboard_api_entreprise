...
const getters = {
  generatePayload(state, getters) {
    ...
    if (getters.adminContactPayload !== null)
      contacts.push(getters.adminContactPayload);
    if (getters.techContactPayload !== null)
      contacts.push(getters.techContactPayload);
  ...
  adminContactPayload(state) {
    if (
      state.user_form.adminContactEmail === "" &&
      state.user_form.adminContactPhone === ""
    ) {
      return null;
    }
  ...
  techContactPayload(state) {
    if (
      state.user_form.techContactEmail === "" &&
      state.user_form.techContactPhone === ""
    ) {
      return null;
    }
