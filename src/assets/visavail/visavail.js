    ...
    selection.each(function drawGraph(dataset) {
      ...
      if (drawTitle) {
        svg.select('#g_title')
            ...
            .text('Historique de disponibilité')
            .attr('class', 'heading');
      }
      ...
      var subtitleText = '';
      if (isDateOnlyFormat) {
        subtitleText = 'du ' + moment(parseDate(startDate)).format('MMMM Y') + ' au '
          + moment(parseDate(endDate)).format('MMMM Y');
      } else {
        subtitleText = 'du ' + moment(parseDateTime(startDate)).format('l') + ' '
            + moment(parseDateTime(startDate)).format('LTS') + ' au '
